...
def parsetask(task: dict):
    ...
    # assert task["extra_fields"]["seed_timestamp"] == "timestamp"
    ...
    if task_type == "retrieval":
        # if the target predicting column in target_type, we need to mask it based on the seed_feature_schema
        # seed_type = target_type + '_' + target_column
        ...
        if target_column_feat in nodemeta[target_type]["feat"]:
            # allowed_feat = [f"{target_type}___{_['name']}" for _ in extrafield["seed_feature_schema"] if _['name'] != target_column]
            allowed_feat = [
                feat for feat in nodemeta[target_type]["feat"] if feat != target_column_feat
            ]
            extra_feat = []
        else:
            # all the features are allowed
            allowed_feat = nodemeta[target_type]["feat"]
            extra_feat = []
    elif task_type == "regression":
        if target_type == seed_type:
            assert seed_type.startswith(target_type)
            # Note: the target_column is the last part of the task name, e.g. "amazon-rating" -> "rating"
            # However, should task care of original tasks may not always satisfy this format
