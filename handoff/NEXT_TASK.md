...
## Single Bounded Task
Execute campaign row `gfm-20260303-r01 / IF-B1 / baseline_validation`: run the first bounded `inference` baseline validation `nsys` slice (with smoke prerequisite), then update campaign/run/result/handoff state.
...
- Phase 4c (`inference`) now needs its first successful baseline-validation `nsys` slice (`IF-B1`) to progress toward the `2/2` gate.
...
## Stopping Criteria
- `IF-B1` has a documented smoke+`nsys` completion in campaign + run logs, or an actionable blocker is documented.
...
- [ ] `IF-B1` smoke run executed or blocked with actionable reason.
- [ ] `IF-B1` baseline `nsys` run executed successfully (or blocked with actionable reason) and artifact path recorded.
- [ ] `profiling/CAMPAIGN_PLAN.md` and `profiling/RUNS.md` are synchronized for `IF-B1`.
