...
| Phase 7: Steady-state annotated profiling capture (final capture set) | in progress | `TR-SA1` completed (`20260304-1541-train-annotated-completion-01`) and `FT-SA1` capture completed (`20260304-1622-finetune-annotated-combine-01`, NVTX coverage verified via forced-export `nvtx_sum` recheck after initial false-negative signal); `IF-SA1` remains. Capture complete only after baseline + steady-state (unannotated + annotated) rows are finished or documented as non-actionable blockers. |
