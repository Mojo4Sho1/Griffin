...
| Phase 3: Baseline profiling slice command verification | in progress | Command/runtime dependencies are validated (`make profiling-preflight` passes, smoke/nsys launch), and dataset assets are now the gating blocker (`datasets/single-pretrain-v3/metanode.yaml` missing). |
