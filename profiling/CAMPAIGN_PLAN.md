...
| gfm-20260304-r02 | train | TR-N1 | ncu_post_review | First realistic-scale train `ncu` hotspot deep dive targeting approved hotspot_1 (`ampere_sgemm_32x32_sliced1x4_tn`). | `hmaintask_completion.py` | `train` | `datasets/single-pretrain-v3-hf` | `-` | `checkpoints/single-completion` | `-` | `-` | `20260318-1752-train-completion-01;20260318-1946-train-completion-01` | blocked | first attempt was correctly blocked by GPU3 occupancy; second attempt launched after GPU3 cleared but failed to collect `ncu` output due `ERR_NVGPUCTRPERM` (GPU performance counters unavailable to current user on target device). Wrapper compatibility issue was also identified and fixed in `scripts/profile_baseline.sh`; dedicated helper `scripts/run_ncu_hotspot.sh` now provides the canonical fresh-run retry path and still depends on host-side counter access via sudo. |
