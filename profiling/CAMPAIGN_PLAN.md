...
- execution_policy_version: `realistic-v2`
- current gate note: asset acquisition/provenance gate is satisfied; `TR-B1`, `FT-B1`, and `IF-B1` are all now `complete` under parity-consistent realistic-v2 scenario-owned multi-slice `nsys` evidence and campaign is ready for cross-scenario review row `RV-R2`.
...
| gfm-20260304-r02 | train | TR-B1 | baseline_validation | Realistic-scale baseline train validation slice gated on production-equivalent assets/provenance. | `hmaintask_completion.py` | `train` | `datasets/single-pretrain-v3-hf` | `-` | `checkpoints/single-completion` | `20260305-2131-train-completion-01;20260306-0049-trb1-realistic-20260306b-s01;20260306-0051-trb1-realistic-20260306b-s02;20260306-0052-trb1-realistic-20260306b-s03` | `20260312-1526-trb1-realistic-20260312a-s01;20260312-1532-trb1-realistic-20260312a-s02;20260312-1537-trb1-realistic-20260312a-s03` | `-` | done | execution_policy_version=`realistic-v2`; legacy_policy_evidence=`false`; slice_size_tier=`8/4`; scenario_completion_state=`complete`; parity rerun completed as scenario-owned multi-slice `nsys` chain with per-slice analysis bundles (`profiling/chains/active/CHAIN_SUMMARY_trb1-realistic-20260312a.md`); legacy standalone run `20260306-1502-trb1-realistic-nsys-01` retained for provenance only. |
...
| gfm-20260304-r02 | campaign | RV-R2 | review_gate | Cross-scenario realistic-scale review after `TR-B1`, `FT-B1`, and `IF-B1` scenario completion states are satisfied. | `-` | `-` | `-` | `-` | `-` | `-` | `-` | `-` | done | execution_policy_version=`realistic-v2`; decision=`pass`; `realistic_cross_scenario_review_complete=true`; `ncu_allowed=true`; `targeted_fine_allowed=false`; `optimization_discussion_allowed=false`; approved hotspot shortlist: (1) `ampere_sgemm_32x32_sliced1x4_tn` ~30-32%, (2) `fmha_cutlassF_f32_aligned_64x64_rf_sm80` ~12.5-13%, (3) `ampere_sgemm_32x128_tn` ~9.3-9.8%; all three present and stable across TR-B1/FT-B1/IF-B1; result recorded in `gfm-20260304-r02-realistic-review-gate-01`. |
| gfm-20260304-r02 | train | TR-N1 | ncu_post_review | First realistic-scale train `ncu` hotspot deep dive targeting approved hotspot_1 (`ampere_sgemm_32x32_sliced1x4_tn`). | `hmaintask_completion.py` | `train` | `datasets/single-pretrain-v3-hf` | `-` | `checkpoints/single-completion` | `-` | `-` | `20260318-1752-train-completion-01;20260318-1946-train-completion-01;20260319-1514-train-completion-01` | done | first attempt was correctly blocked by GPU3 occupancy; second attempt launched after GPU3 cleared but failed to collect `ncu` output due `ERR_NVGPUCTRPERM`; third attempt was executed under user-run `sudo` and produced a valid but oversized `7.8G` legacy `.ncu-rep` artifact. On `2026-03-24`, the upgraded no-timeout `tmux` re-analysis completed successfully, generated stable section sidecars for the original seven core Nsight Compute views, then added `WorkloadDistribution` as the new eighth default core section and proved resumable reuse after interruption. Future hotspot captures should default to `core + cap` (`launch-count=5`; tooling smoke usually `1`) and use `--set full` only as explicit escalation. |
| gfm-20260304-r02 | train | TR-N2 | ncu_post_review | Second realistic-scale train `ncu` hotspot deep dive targeting approved hotspot_2 (`fmha_cutlassF_f32_aligned_64x64_rf_sm80`) using the user-started `tmux` workflow by default. | `hmaintask_completion.py` | `train` | `datasets/single-pretrain-v3-hf` | `-` | `checkpoints/single-completion` | `-` | `-` | `-` | not started | resume this row now that `TR-N1` bundle validation is complete; default command path is `scripts/run_ncu_hotspot.sh train hotspot_2` with `--sections-profile core` and `--launch-count 5`, using detached/reattachable `tmux` after the user has entered `sudo`. Optional `ncu_intent: tooling_smoke` validation pass should generally use `--launch-count 1` before any escalation. |
