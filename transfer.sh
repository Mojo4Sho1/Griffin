...
# GPU can be one number or a list of numbers splitted by comma
# Find the last number in the GPU list
GPU_last=${GPU##*,}
# Find the number of GPUs by counting the commas
GPU_num=$(($(echo $GPU | tr -cd ',' | wc -c) + 1))
EPOCH=200

# Predefined downsample nums: 512 4096
